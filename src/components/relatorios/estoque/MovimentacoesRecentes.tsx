...
import { formatDateBRL } from "@/utils/date";

interface MovimentacoesRecentesProps {
  movimentacoes: MovimentacaoResumo[];
}

export function MovimentacoesRecentes({ movimentacoes }: MovimentacoesRecentesProps) {
              ...
              movimentacoes.map((mov) => (
                <tr key={mov.id} className="hover:bg-gray-50 transition-colors">
                  <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-500 sm:pl-6">
                    {formatDateBRL(String(mov.dataMovimentacao))}
                  </td>
